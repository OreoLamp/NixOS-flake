{ pkgs, ... }:
{
    systemd = {
        ...
        {
            mountConfig = { DirectoryMode = "0755"; };
            description = "Windows partition mount";
            options = "nofail,atime,exec,windows_names,remove_hiberfile,big_writes,uid=1000,gid=100,umask=0022,fmask=0022,gmask=0022";
            wants = [ "multi-user.target" ];
            wantedBy = [ "multi-user.target"];
            after = [ "multi-user.target" ];
            type = "ntfs-3g";
            what = "/dev/disk/by-uuid/4818C70318C6EF4E";
            where = "/media/win11";
        }
        {
            mountConfig = { DirectoryMode = "0755"; };
            description = "WSL btrfs partition mount";
            options = "defaults,nofail";
            wants = [ "multi-user.target" ];
            wantedBy = [ "multi-user.target"];
            after = [ "multi-user.target" ];
            type = "btrfs";
            what = "/dev/disk/by-uuid/17f35653-b47c-46ed-ac85-a749759b2563";
            where = "/media/wsl";
        }];
