...
let
    ...
    makeCacheConf = { }:
        let
            makeCache   = fontconfig: pkgs.makeFontsCache { 
                inherit fontconfig; fontDirectories = config.fonts.packages; 
            };
    ...
    # local configuration file, priority 51 (technically, it's weird)
    ...
    replaceDefaultConfig = defaultConfig: newConfig: ''
        ...
        ln -s ${pkg.out}/share/fontconfig/conf.avail/${newConfig} $dst/
    '';
    ...
    ''
        ...
        ln -s /etc/fonts/fonts.conf $out/etc/fonts/2.11/fonts.conf
        ...
        # 50-user.conf
        ${optionalString (!cfg.includeUserConf) "rm $dst/50-user.conf" }
                    ...
                    style = mkOption {
                        ...
                        apply =
                            val: let
                                ...
                                warning = "The option `${from}` contains a deprecated value `${val}`. ''\
                                Use `${val'}` instead.";
                            in
